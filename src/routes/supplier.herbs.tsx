...
import { herbs as seed, getNursery } from "@/data/mock";
          ...
          <TableBody>
            {rows.map((h) => (
              <TableRow key={h.slug}>
                ...
                <TableCell className="font-medium">{h.name}{h.local ? ` · ${h.local}` : ""}</TableCell>
                <TableCell><Badge variant="secondary" className="rounded-full">{h.category}</Badge></TableCell>
                <TableCell className="text-xs text-muted-foreground">{h.nurseryIds.map((id: string) => getNursery(id)?.name).filter(Boolean).join(", ")}</TableCell>
                <TableCell>{h.care.difficulty}</TableCell>
