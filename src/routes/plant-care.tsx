...
import { fetchHerbs } from "@/lib/queries";
...
function PlantCare() {
  ...
  useEffect(() => { fetchHerbs().then(setHerbs).catch(() => {}); }, []);
  return (
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {herbs.map((h) => (
          <Link key={h.id} to="/browse-herbs/$slug" params={{ slug: h.slug }}
            ...
            <div className="aspect-square overflow-hidden rounded-2xl bg-secondary">
              <img src={resolveHerbImage(h.image_url, h.slug)} alt={h.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
            </div>
            <div className="min-w-0">
              ...
              <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground">
                {h.care.sunlight && <li className="flex items-center gap-2"><Sun className="h-4 w-4 text-primary" /> {h.care.sunlight}</li>}
                {h.care.watering && <li className="flex items-center gap-2"><Droplet className="h-4 w-4 text-primary" /> {h.care.watering}</li>}
                {h.care.harvest && <li className="flex items-center gap-2"><Scissors className="h-4 w-4 text-primary" /> Harvest in {h.care.harvest}</li>}
                {h.care.timeline && <li className="flex items-center gap-2"><Sprout className="h-4 w-4 text-primary" /> {h.care.timeline}</li>}
              </ul>
            </div>
          </Link>
        ))}
