...
export const Route = createFileRoute("/nurseries")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Explore partner nurseries stocking fresh kitchen herbs." },
    ],
  }),
...
function NurseriesPage() {
  ...
  useEffect(() => {
    Promise.all([fetchSuppliers(), fetchHerbs()]).then(([s, h]) => { setSuppliers(s); setHerbs(h); });
  }, []);
  return (
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {suppliers.map((n) => {
          const nherbs = herbs.filter((h) => h.supplier_id === n.id);
          return (
            <article key={n.id} className="group flex flex-col overflow-hidden rounded-3xl border bg-card shadow-sm transition-shadow hover:shadow-xl">
              <div className="aspect-[4/3] overflow-hidden">
                <img src={resolveSupplierImage(n.image_url, n.slug)} alt={n.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
              </div>
              <div className="flex flex-1 flex-col gap-3 p-5">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-display text-xl font-semibold">{n.name}</h3>
                  <span className="flex shrink-0 items-center gap-1 text-sm text-primary"><Star className="h-4 w-4 fill-current" /> {n.rating}</span>
                </div>
                ...
                <div className="mt-1 flex flex-wrap gap-1.5">
                  {nherbs.slice(0, 4).map((h) => (
                    <span key={h.id} className="rounded-full bg-secondary px-2.5 py-0.5 text-xs">{h.name}</span>
                  ))}
                </div>
                {n.hours && <p className="mt-1 text-xs text-muted-foreground">Open {n.hours}</p>}
